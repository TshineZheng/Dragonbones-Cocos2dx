...
class NativeClass(object):
    ...
    def fields_clean(self):
        ...
        for m in self.public_fields:
            should_bind = self.generator.should_bind_field(self.class_name, m.name)
            if should_bind:
                print "bindfield",self.class_name,m.name
                ret.append({"name": m.name, "signature_name": m.signature_name})
        return ret
